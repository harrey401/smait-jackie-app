...
class CaeAudioManager(private val context: Context) {
    ...
    fun start(ws: WebSocket) {
        ...
        try {
            ...
            // Kill only audio.service (iFLYTEK vendor) if it holds ALSA.
            // Do NOT kill other apps (e.g. com.gow.smaitrobot) — they share
            // the device via foreground/background lifecycle instead.
            try {
                val lsof = Runtime.getRuntime().exec(arrayOf("su", "0", "sh", "-c",
                    "lsof /dev/snd/pcmC${PCM_CARD}D${PCM_DEVICE}c 2>/dev/null | tail -n +2"
                ))
                val lines = lsof.inputStream.bufferedReader().readText().trim()
                lsof.waitFor()
                if (lines.isNotEmpty()) {
                    for (line in lines.lines()) {
                        val parts = line.trim().split("\\s+".toRegex())
                        val procName = parts.getOrNull(0) ?: continue
                        val pid = parts.getOrNull(1) ?: continue
                        // Only kill system audio services, not other SMAIT apps
                        if (procName == "audio.se" || procName.startsWith("audio")) {
                            Runtime.getRuntime().exec(arrayOf("su", "0", "kill", pid)).waitFor()
                            Log.i(TAG, "Killed audio.service ALSA holder PID $pid")
                        } else {
                            Log.i(TAG, "Skipping ALSA holder: $procName (PID $pid) — not audio.service")
                        }
                    }
                    Thread.sleep(500)
                } else {
